...
启动 web服务 ，在localhost:8000打开测试页面
...
def _process_event_stream(event):
    ...
    if hasattr(event, 'content') and event.content and hasattr(event.content, 'parts'):
        for part in event.content.parts:
            ...
            if hasattr(part, 'function_call') and part.function_call:
                ...
                # logger.tool_call(fc.name, fc.args)
            ...
            if hasattr(part, 'function_response') and part.function_response:
                ...
                chunks.append({"type": "tool_result", "content": fc_tool_response_msg})
    ...
    if hasattr(event, 'is_final_response') and event.is_final_response():
        if event.content and event.content.parts:
            ...
            # Final text is already streamed via chunks, so we don't need to append it again.
            # chunks.append({"type": "text", "content": final_text})
            pass
...
async def run_agent(task: str):
    ...
    try:
        async for event in runner.run_async(
            ...
            for chunk in chunks:
                full_final_result_list.append(chunk)
                yield chunk
    except Exception as e:
...
async def chat_endpoint(request: ChatRequest):
    ...
    async def generate():
        # First yield the app name as a special event or just part of the stream?
        # For simplicity, let's just stream the content. The frontend knows the app name from previous context or we can send it first.
        # But the user wants "app_name" in the response.
        # With StreamingResponse, we can't easily return a JSON with metadata AND a stream.
        # We will stream JSON chunks.
        yield json.dumps({"app_name": DEFAULT_APP_NAME}) + "\n"
        
        async for chunk in run_agent(request.message):
